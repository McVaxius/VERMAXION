...
With the **VerMAXION + AutoHook** provider, fixed-spot rail mode assigns a spot deterministically from a settled passenger roster. It prefers unclaimed clear spots when the assignment is unavailable, then searches forward from the assigned index; spot clearance is capped by the closest pair in the built-in list. Continuous rail mode retains its separate sampling and clearance policy. Positioning waits through `WaitingForPlayers` and zone transitions, and the initial arrival remains subject to clearance checks for three seconds. Placement cannot exhaust its retry budget before two minutes have elapsed. Passenger identity is used internally for assignment but is not included in the roster-assignment diagnostic.

Reaching a fishing point stops vnavmesh and applies outward character rotation—the camera is not rotated. Casting still waits for path-stop, facing, and placement readiness, then sends `/ahstart` and `/ac cast` together on the existing retry cadence. A valid Fishing/Gathering acknowledgement locks normal voyage movement. The exception is full-inventory recovery: the exact English game message `Unable to gather. Insufficient inventory space.` starts an onboard Merchant & Mender sell attempt through `/ays itemsell`, waits for AutoRetainer busy then idle, and returns to the saved fishing position and facing. Unavailable merchant/state and return navigation have timeout recovery paths; an AutoRetainer operation that remains busy is still awaited. Voyage completion interrupts this recovery for result handling. Other client-language messages are not matched.
