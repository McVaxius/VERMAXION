...
During initial Ocean Fishing positioning, VERMAXION samples a continuous point from the proven port or starboard rail ranges instead of choosing a fixed slot. Candidates within three yalms of another player are rejected; an exhausted 32-candidate pass stops navigation, blocks casting, and retries after one second. Reaching the point within 0.5 yalms stops vnavmesh and applies outward character rotation—the camera is not rotated. `/ahstart` and Fishing/Gathering acknowledgement remain locked until live player clearance is still at least three yalms, `vnavmesh.Path.IsRunning` has remained false for one continuous second, and character-facing readback is within 0.05 radians. Clearance loss or bounded recovery selects a fresh continuous point before the first valid acknowledgement. That acknowledgement permanently locks voyage movement, so later route changes and fishing interruptions retry `/ahstart` in place without respreading.
