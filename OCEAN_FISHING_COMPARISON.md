...
| Fishing sessions | At each seven-minute session start, Versatile Lure is set once and `/ahstart` is immediately eligible, then retried every 3 seconds until Fishing/Gathering acknowledgement. Retries continue during movement and after arrival. Arrival is within 0.5 yalms: navigation stops, canonical facing is applied, 500 ms is allowed for settlement, then facing may be reapplied at most once per second until acknowledgement. AutoHook presets remain unmanaged. |
| Voyage movement boundary | Before fishing has ever acknowledged, VERMAXION cycles and wraps through all six fixed rails after 10 active seconds without 0.25 yalms of progress, 30 active navigation seconds on one spot, 10 available/non-busy post-arrival seconds with `CanFish` false, or five post-arrival `/ahstart` attempts without acknowledgement. Recovery timers pause for route transitions, unavailable-player states, combat, casting, and occupied states; each advance stops vnavmesh and resets only destination-level recovery state. The first Fishing/Gathering acknowledgement stops navigation immediately and permanently locks movement for the voyage. Later interruptions and route changes retry `/ahstart` in place. |
